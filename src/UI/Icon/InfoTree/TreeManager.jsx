...
class TreeManager extends React.Component {
  ...
  componentDidMount() {
    ...
    Promise.all(_info.map(
      model => api.getTreeList(this.props.viewer, model.key, apiVersion, isOffline))
    )
      .then(rsp => {
        rsp.forEach((r, i) => {
  ...
  getTreeData(type, force = false) {
    ...
    if (force || !this.data[type]) {
      ...
      Promise.all(modelInfo[type].map(
        info => api.getData(
          this.props.viewer.host, info.fileKey, info.modelKey, this.props.isOffline
        )
      ))
        .then(rsp => {
          // 当前数据类型的数据置空
        ...
        if (data.length > 0) {
          content = (
            <Tree
              ...
              isShow={show}
              onCheck={(state, cptKeys) => { this.onCheckCpts(state, cptKeys) }}
              // onClickLeaf={cptKeys => { this.onClickCpt(cptKeys) }}
              queryFn={fn => { this.treeInfo[type].queryFn = fn }}
              queryIncFn={fn => { this.treeInfo[type].queryIncFn = fn }}
  ...
  render() {
    ...
    _.uniq(Object.keys(modelInfo).concat(DEFAULT_TYPES)).forEach(type => {
      if (!DEFAULT_TYPES.includes(type) && type !== '族树' && type !== "部件树") {
        otherOpts.push(type);
      }
        ...
        >
          <div className={style.tree}>
            <Query
              ...
              onSearchPrev={() => { this.queryInc(treeType, -1) }}
              onSearchNext={() => { this.queryInc(treeType, 1) }}
